...
// Generic functions for workspace role assignments
async function listWorkspaceRoleAssignments(creds: any, workspaceId: number) {
  const response = await fetch(`${creds.baseUrl}/api/workspaces/${workspaceId}/role-assignments/`, {
...
async function createWorkspaceRoleAssignment(creds: any, workspaceId: number, data: any) {
  const response = await fetch(`${creds.baseUrl}/api/workspaces/${workspaceId}/role-assignments/`, {
    method: "POST",
    headers: {
...
async function deleteWorkspaceRoleAssignment(creds: any, assignmentId: number) {
...
const ROLES = [
  { value: "ADMIN", label: "Admin", description: "Full access to workspace settings and all applications." },
  { value: "MEMBER", label: "Member", description: "Can manage databases and data within applications." },
  { value: "VIEWER", label: "Viewer", description: "Read-only access to all applications in workspace." },
  { value: "NO_ACCESS", label: "No Access", description: "Cannot see this workspace at all." },
];

export default function WorkspacePermissionsScreen() {
  const colors = useColors();
  const creds = useCreds();
  const { apiCall } = useAuth();
  const queryClient = useQueryClient();
  const params = useLocalSearchParams<{ id: string; workspaceName?: string }>();
  ...
  const assignmentsQuery = useQuery({
    queryKey: ["workspace-role-assignments", workspaceId],
    queryFn: () => listWorkspaceRoleAssignments(creds, workspaceId),
    enabled: !!workspaceId,
  });

  const createMutation = useMutation({
    mutationFn: (data: any) => createWorkspaceRoleAssignment(creds, workspaceId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["workspace-role-assignments", workspaceId] });
      setModalOpen(false);
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteWorkspaceRoleAssignment(creds, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["workspace-role-assignments", workspaceId] });
    },
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Stack.Screen options={{ title: `Permissions: ${workspaceName}` }} />
        ...
        ListHeaderComponent={() => (
          <View style={styles.header}>
            <Text style={[styles.headerText, { color: colors.mutedForeground }]}>
              Manage workspace-level role assignments. These overrides allow for precise control over who can manage this workspace.
            </Text>
          </View>
        )}
        renderItem={({ item }) => (
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.background }]}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, { color: colors.foreground }]}>Assign Workspace Role</Text>
