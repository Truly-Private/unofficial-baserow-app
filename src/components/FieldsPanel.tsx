...
// Simple colors hook (lightweight version)
function useColors() {
  return {
    background: "#fff",
    foreground: "#000",
...
export function FieldsPanel({
  ...
  const fieldsQuery = useQuery({
    queryKey: ["fields", client.baseUrl, tableId],
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["fields", client.baseUrl, tableId] });
